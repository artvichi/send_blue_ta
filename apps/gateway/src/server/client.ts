 ...
 */
export async function claimLease(signal?: AbortSignal): Promise<LeaseDto | null> {
  const wait = config().GATEWAY_POLL_WAIT_SECONDS;
  let response: Response;
  try {
    response = await request(`/api/gateway/lease?wait=${wait}`, { signal });
  } catch (err) {
    // Shutdown cancels an idle long-poll; that is not a failure, just "nothing".
